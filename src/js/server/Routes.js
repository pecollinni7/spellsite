...
const getData = () => needle.request('get', Settings.CHECK_FOR_UPDATES,
    ...
    if (response.statusCode === 200)
    {
        ...
        fs.writeFile(Settings.getSettings('path.data'), JSON.stringify(body, null, 4), 'utf8', function (err) {
            if (err)
            {
...
{
    return fs.readdirSync(Settings.getSettings('path.media'), {withFileTypes: true}).filter(item => !item.isDirectory()).map(item => item.name);
}
...
const setData = (cb) => needle.request('post', Settings.SET_DATA_FILE,
    ...
    if (response.statusCode === 200)
    {
        ...
        fs.writeFile(Settings.getSettings('path.data'), JSON.stringify(body, null, 2), 'utf8', function (err) {
            if (err)
            {
                console.log('error while writing poller response');
                throw err;
...
const uploadFile = (data) => {
    ...
    needle.request('post', Settings.UPLOAD_FILE, {file: data}, {multipart: true}, (error, response, body) => {
